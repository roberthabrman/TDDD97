...
def sign_in():
    ...
    if FoundUser is not None:
        ...
        salt = userinfo[7]#gets the salt



        passwordsent = password + salt#this is how we will generate the salt


        if bcrypt.check_password_hash(realpassword,passwordsent):#compares the stored password that contains the salt+password with the sentpassword+salt
        ...
        return json.dumps ({"success": False, "message": "You are not signed up."})
...
def sign_up():
    ...
    if database_helper.GetUser(email) == None:
        ...
        salt = uuid.uuid4().hex#this is how we will generate the salt

        pw_hash = bcrypt.generate_password_hash(password+salt)#we add the salt to the stored password
        ...
        database_helper.AddUser(email,pw_hash,firstname,familyname,gender,city,country, salt)#we also add the specified salt to the user
...
def change_password():
    ...
    if not tempone:
        ...
        if hashserver == hashclient:
            ...
            salt = database_helper.GetUser(email)[7]#we retrieve the salt
            passwordsent = oldpassword + salt#we add the salt to the sentpassword


            if bcrypt.check_password_hash(currentpassword,passwordsent):#we compare the two

                pw_hash = bcrypt.generate_password_hash(newpassword+salt)#we also add the salt to the new password that was sent.
            ...
            return json.dumps({"success": False, "message": "Invalid authentication"})
