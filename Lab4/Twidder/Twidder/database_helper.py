...
def connect_db ():
    conn = sqlite3.connect("/home/salsa083/PycharmProjects/Lab3/Twidder/Twidder/database.db")
    c = conn.cursor()
    return sqlite3.connect("/home/salsa083/PycharmProjects/Lab3/Twidder/Twidder/database.db")
...
def get_db():
    ...
    """
    if not hasattr(g, '/home/salsa083/PycharmProjects/Lab3/Twidder/Twidder/database_db'):
...
def close_db(error):
    ...
    if hasattr(g, '/home/salsa083/PycharmProjects/Lab3/Twidder/Twidder/database.db'):
        g.sqlite_db.close()
...
def AddUser(email, password, firstname, familyname, gender, city, country, salt):
    user = query_db('INSERT INTO user VALUES (?, ?, ?, ?, ?, ?, ?, ?)',[email, password, firstname, familyname, gender, city, country, salt])
    print user
    return
...
def GetLoggedInUserToken(token):
    user = query_db('SELECT * FROM loggedinusers WHERE loggedinusers.token = (?)',[token],one=True)
    return (user)
...
def DeleteLoggedInUserEmail(email):
    query_db('Delete FROM loggedinusers WHERE loggedinusers.email = (?)', [email])
    return
